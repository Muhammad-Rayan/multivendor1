      ...
      <div class="card-header border-0 pt-5">
          ...
          <div class="col col-4">
              <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">Refund Request</span>
              </h3>
          </div>
          <div class="col col-8">
            <div class="row">
          ...
          <tbody>
            <template v-for="(item, index) in model.data" :key="index">
              <tr>
                ...
                <td>
                  <span class="badge badge-light-primary" v-if="item.status == 1">Refund</span>
                </td>
                <td>
...
export default ({
  ...
  methods: {
    ...
    updatePerPage() {
      ...
      get(`/api/refund?status=1&per_page=${this.params.per_page}&q=${this.params.q}`)
      .then(res => {
          this.setData(res)
      })
    },
    nextPage() {
        if(this.model.next_page_url) {
            ...
            get(`/api/refund?status=1&per_page=${this.params.per_page}&page=${this.params.page}&q=${this.params.q}`)
            .then(res => {
                this.setData(res)
            })
        }
