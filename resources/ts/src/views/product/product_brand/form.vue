    ...
    <div :class="widgetClasses" class="card">
        ...
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">{{title}} New Brand</span>
            </h3>
        </div>
...
function initializeUrl (to) {
    let urls = {
        'create': `/api/product/brand/create`,
        'edit': `/api/product/brand/${to.params.id}/edit`,
        'clone': `/api/product/brand/${to.params.id}/edit?mode=clone`,
    }
...
export default ({
  ...
  data() {
        return {
            store: '/api/product/brand',
            method: 'POST',
            title: 'Create',
            message : 'You have successfully create Brand!',
            form: {},
        }
    },
  created() {
      if(this.mode === 'edit') {
          this.store = `/api/product/brand/${this.$route.params.id}?_method=PUT`
          this.message = 'You have successfully updated Brand!'
          this.method = 'POST'
          this.title = 'Edit'
      }
  },
