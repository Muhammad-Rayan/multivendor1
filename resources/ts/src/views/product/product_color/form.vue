    ...
    <div :class="widgetClasses" class="card">
        ...
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">Add New Color</span>
            </h3>
        </div>
...
function initializeUrl (to) {
    let urls = {
        'create': `/api/product/color/create`,
        'edit': `/api/product/color/${to.params.id}/edit`,
        'clone': `/api/product/color/${to.params.id}/edit?mode=clone`,
    }
...
export default ({
 ...
 components: { Dropdown2,FileUpload,ErrorText },
  ...
  created() {
      if(this.mode === 'edit') {
          this.store = `/api/product/color/${this.$route.params.id}?_method=PUT`
          this.message = 'You have successfully updated Color!'
          this.method = 'POST'
          this.title = 'Edit'
      }
  },
  ...
  methods: {
    save() {
      this.submitMultipartForm(this.form,(data) => {
          ...
          this.$router.push({ name: 'product-color' });
        })
    },
    onDocument(e) {
        console.log(e);
