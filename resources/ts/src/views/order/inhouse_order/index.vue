      ...
      <div class="card-header border-0 pt-5">
          ...
          <div class="col col-2">
              <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">In House Orders</span>
              </h3>
          </div>
          <div class="col col-8">
            <div class="row">
...
export default ({
  ...
  data() {
        return {
            model: [],
            resource:'/api/order/inhouseorder',
            params: {
              per_page: 12,
              page: 1,
              q: '',
  ...
  methods: {
    ...
    updatePerPage() {
      ...
      get(`/api/order/inhouseorder?per_page=${this.params.per_page}&q=${this.params.q}`)
      .then(res => {
          this.setData(res)
      })
    },
    nextPage() {
        if(this.model.next_page_url) {
            ...
            get(`/api/order/inhouseorder?per_page=${this.params.per_page}&page=${this.params.page}&q=${this.params.q}`)
            .then(res => {
                this.setData(res)
            })
        }
    ...
    deleteItems(id) {
        byMethod('POST', `/api/order/inhouseorder/${id}/delete`)
        .then(({data}) => {
